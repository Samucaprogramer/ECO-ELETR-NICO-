...
def home_screen():
    ...
    <div style='text-align: center; padding: 40px;'>
        <h2 style='color: #667eea;'>Bem-vindo ao Eco Eletrônico!</h2>
...
def cadastro_screen():
    ...
    with col1:
        if st.button("Cadastrar", key="btn_do_cadastro", use_container_width=True):
            ...
            elif find_user(nome, turma):
                st.error("❌ Usuário já cadastrado! Faça login.")
            else:
                novo_user = {
...
def login_screen():
    ...
    nome = st.text_input("Nome Completo (exatamente como cadastrou)", key="login_nome")
        ...
        if st.button("Entrar", key="btn_do_login", use_container_width=True):
            if not nome or turma == 'Selecione...':
                ...
                if user:
                    ...
                    st.error("❌ Usuário não encontrado! Verifique se o nome está exatamente como cadastrou.")
    with col2:
